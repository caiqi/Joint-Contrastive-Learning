...
class MoCoUnlimitedKeysDefault(nn.Module):
    """
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, mlp=False):
        ...
        """
        super(MoCoUnlimitedKeysDefault, self).__init__()
    ...
    def forward(self, im_q, im_k, ratio=1.0):
        """
        Input:
        ...
        N, C = q.shape
        isda_loss = 0.5 * torch.bmm(torch.bmm(q.reshape(N, 1, C), k_sigma), q.reshape(N, C, 1)).mean() / self.T
        return logits, labels, isda_loss
